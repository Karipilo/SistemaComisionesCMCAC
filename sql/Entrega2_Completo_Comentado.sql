...
CREATE OR REPLACE FUNCTION FN_OBTENER_COSTO_EMPRESA (
        ...
        -- RUT del auditor (con guión)
        p_mes IN NUMBER,
        -- Mes del proceso (1 al 12)
...
-- Si no encuentra dato, devuelve 0
EXCEPTION
...
CREATE OR REPLACE PROCEDURE PRC_LISTAR_COMISIONES IS CURSOR cur_comisiones IS -- Cursor que lista los datos básicos de comisiones
SELECT run_auditor,
    nombre_auditor,
...
v_total NUMBER;
BEGIN OPEN cur_comisiones;
LOOP FETCH cur_comisiones INTO v_run,
v_nombre,
v_total;
EXIT
WHEN cur_comisiones %NOTFOUND;
-- Aquí se podría almacenar en una tabla intermedia, mostrar en frontend o retornar
NULL;
END LOOP;
CLOSE cur_comisiones;
EXCEPTION
WHEN OTHERS THEN -- Captura el error y lo registra en tabla de errores
...
CREATE OR REPLACE PACKAGE PKG_REPORTE_COMISION IS TYPE t_cursor IS REF CURSOR;
-- Definición del tipo cursor dinámico
...
/
CREATE OR REPLACE PACKAGE BODY PKG_REPORTE_COMISION IS PROCEDURE PRC_REPORTE_POR_PROFESION (
        p_mes IN NUMBER,
        p_anno IN NUMBER,
        p_reporte OUT t_cursor
    ) IS BEGIN OPEN p_reporte FOR
...
ORDER BY 2 DESC;
EXCEPTION
WHEN OTHERS THEN -- Captura error y lo registra
INSERT INTO ERROR_PROCESO (programa_origen, sql_code, sql_mensaje)
...
CREATE OR REPLACE PROCEDURE PRC_LIMPIAR_COMISIONES IS v_mes_proceso NUMBER := 8;
v_anno_proceso NUMBER := 2021;
BEGIN
DELETE FROM DETALLE_COMISIONES_AUDITORIAS_MES
WHERE mes_proceso = v_mes_proceso
    AND anno_proceso = v_anno_proceso;
EXCEPTION
WHEN OTHERS THEN
DECLARE v_code NUMBER := SQLCODE;
...
SET comision_extra = (
        SELECT ROUND(
                A.sueldo * (
                    (NVL(TC.porc_incentivo, 0) + p_nuevo_factor) / 100
                ),
                2
            )
        FROM auditor A
...
SELECT COUNT(run_auditor),
    ...
    NVL(SUM(total_comision_empresa), 0) INTO v_total_auditores,
    v_suma_comision,
    v_suma_costo
FROM DETALLE_COMISIONES_AUDITORIAS_MES
WHERE mes_proceso = p_mes_proceso
...
CREATE OR REPLACE TRIGGER TRG_CALCULAR_RECARGO_EMP BEFORE
INSERT
    OR
UPDATE OF total_comision_audit ON DETALLE_COMISIONES_AUDITORIAS_MES FOR EACH ROW
DECLARE v_msg VARCHAR2(4000);
BEGIN :NEW.total_comision_empresa := ROUND(NVL(:NEW.total_comision_audit, 0) * 1.10, 2);
IF INSERTING THEN v_msg := 'INSERT -> total_comision_audit: ' || NVL(:NEW.total_comision_audit, 0) || ' | total_comision_empresa: ' || NVL(:NEW.total_comision_empresa, 0);
INSERT INTO ERROR_PROCESO (programa_origen, sql_code, sql_mensaje)
VALUES ('TRG_CALCULAR_RECARGO_EMP (INSERT)', 0, v_msg);
END IF;
IF UPDATING THEN v_msg := 'UPDATE -> OLD total_comision_audit: ' || NVL(:OLD.total_comision_audit, 0) || ' | NEW total_comision_audit: ' || NVL(:NEW.total_comision_audit, 0) || ' | NEW total_comision_empresa: ' || NVL(:NEW.total_comision_empresa, 0);
INSERT INTO ERROR_PROCESO (programa_origen, sql_code, sql_mensaje)
...
WHEN OTHERS THEN RAISE_APPLICATION_ERROR(
    -20001,
    'Error en TRG_CALCULAR_RECARGO_EMP: ' || SQLERRM
);
END TRG_CALCULAR_RECARGO_EMP;
/ -- 4. BLOQUE PRINCIPAL DE PROCESAMIENTO
DECLARE v_mes_proceso NUMBER := 8;
v_anno_proceso NUMBER := 2021;
BEGIN PRC_LIMPIAR_COMISIONES;
COMMIT;
...
SELECT v_mes_proceso,
    v_anno_proceso,
    run_auditor,
    MAX(nombre_auditor),
    MAX(nombre_profesion),
    MAX(comision_total_audit),
    MAX(comision_monto_audit),
    MAX(comision_prof_critica),
    MAX(comision_extra),
    ROUND(
        MAX(comision_total_audit) + MAX(comision_monto_audit) + MAX(comision_prof_critica) + MAX(comision_extra),
        2
    )
FROM (
        SELECT A.numrun || '-' || A.dvrun AS run_auditor,
            A.nombre || ' ' || A.appaterno || ' ' || A.apmaterno AS nombre_auditor,
            P.nombre_profesion,
            ROUND(
                T_AUDIT.monto_total_auditorias * (NVL(P_TOTAL.porc_total_audit, 0) / 100),
                2
            ) AS comision_total_audit,
            ROUND(
                T_AUDIT.monto_total_auditorias * NVL(P_MONTO.porc_monto_audit, 0),
                2
            ) AS comision_monto_audit,
            ROUND(
                CASE
                    P.nivel_criticidad
                    WHEN 1 THEN A.sueldo * 0.05
                    WHEN 2 THEN A.sueldo * 0.03
                    WHEN 3 THEN A.sueldo * 0.01
                    WHEN 4 THEN A.sueldo * 0.005
                    ELSE 0
                END,
                2
            ) AS comision_prof_critica,
        ...
        FROM auditor A
            ...
            INNER JOIN (
                SELECT id_auditor,
                    COUNT(id_auditor) AS total_auditorias,
            ...
            LEFT JOIN porc_total_auditorias P_TOTAL ON T_AUDIT.total_auditorias BETWEEN P_TOTAL.total_audit_min AND P_TOTAL.total_audit_max
            LEFT JOIN porc_monto_auditorias P_MONTO ON T_AUDIT.monto_total_auditorias BETWEEN P_MONTO.monto_audit_min AND P_MONTO.monto_audit_max
    ) S
GROUP BY run_auditor,
    v_mes_proceso,
    v_anno_proceso;
COMMIT;
...
v_anno_proceso NUMBER := 2021;
v_dummy NUMBER;
BEGIN -- Error intencional: división por cero
...
END;
/
SELECT '--- REGISTRO DE ERROR_PROCESO (Debe mostrar el error ORA-01476) ---' AS MENSAJE
FROM DUAL;
SELECT id_error,
    programa_origen,
    fecha_error,
